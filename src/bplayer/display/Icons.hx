...
class Icons {
	...
	public static function volumeIcon(w:Int, h:Int, i:Int, ?f:Path->Void):Document {
		if (!i.inRange(0, 3)) {
			throw 'Error: icon "volume$i" does not exist';
		}
		return namedPath(w, h, 'sound$i', f);
	}
	public static function volumeIcons(w:Int, h:Int, ?f:Path->Void):Array<Document> {
		return [for (i in 0...4) volumeIcon(w, h, i, f)];
	}
